...
Gateway.prototype.reportMetrics = function reportMetrics( ) {
    ...
    // sort the samples into timestamp order to report the most recent
    // this.samples contains the unreported metrics, this.previousValues the cached metrics
    // Metrics are cached to allow supersampling, ie polling more frequently than updated.
    this.samples.sort(function(a, b) { return a.ts <= b.ts ? -1 : +1 });
    var samples = this.samples.splice(0);
    ...
    // here we discard from the as-yet unreported `samples`, below from the cached `previousValues`
    var now = this.getTimestamp();
    var oldestKeepTs = this.maxMetricAgeMs >= 0 ? (now - this.maxMetricAgeMs) : 0;
    var filterOldMetrics = this.maxMetricAgeMs >= 0;
    if (filterOldMetrics) {
    ...
    // gather the samples by name,
    // and average them for smoother subsampling for when polling less frequently than updated
    var id, map = {}, averages = new Array();
    samples.forEach(function(metric) {
        id = metric.id;
        if (!map[id]) {
    ...
    // Use `maxMetricAgeMs` to age out metrics too old to be of interest.

    // TODO: `reportOnce` option to discard already reported metrics (ie no supersampling)
    // if (this.reportOnce) this.previousValues = {};

    var discardedMetrics = 0;
    var allValues = {};
    for (id in this.previousValues) {
        if (filterOldMetrics && this.previousValues[id].ts < oldestKeepTs) discardedMetrics += 1;
        else allValues[id] = this.previousValues[id];
    }
