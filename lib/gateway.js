...
Gateway.prototype.parseMetricsLines = function parseMetricsLines( lines, badLines, goodLines, samples ) {
    ...
    for (i=0; i<lines.length; i++) {
        ...
        if (line[0] === '#') {
            // HELP and TYPE describe the adjacent metrics until the first blank line
            if (line.indexOf('# HELP') === 0) help = line;
            if (line.indexOf('# TYPE') === 0) type = line;
            // TODO: parse and hash help/type by name, ie '# TYPE {name} {type}'
            // TODO: retain type/help info, reuse until changed
            continue;
        }
...
Gateway.prototype.reportMetrics = function reportMetrics( ) {
    if (this.readPromMetrics) this.ingestMetrics(this.readPromMetrics(), function(){});
    ...
    samples.forEach((metric) => {
        id = metric.id;
        if (!map[id]) {
            map[id] = { id: id, name: metric.name, value: 0, labels: metric.labels, ts: 0, count: 0, help: null, type: null };
        ...
        map[id].ts = +metric.ts;
        map[id].help = map[id].help || metric.help;
        map[id].type = map[id].type || metric.type;
    })
...
Gateway.prototype.reportAllValues = function reportAllValues( allValues ) {
    ...
    for (i=0; i<names.length; i++) {
        name = names[i];
        reportLines.push(nameIdMap[name][0].help || util.format('# HELP %s custom metric', name));
        reportLines.push(nameIdMap[name][0].type || util.format('# TYPE %s gauge', name));
        for (j=0; j<nameIdMap[name].length; j++) {
            metric = nameIdMap[name][j];
            id = metric.name;
            if (this.labels || metric.labels) id += '{' + this.labels + metric.labels + '}';
            reportLines.push(util.format('%s %s %s', id, metric.value, metric.ts));
        }
        reportLines.push('');
    }
